...
class UPIPayment(PaymentProcessor):
    ...
    def process_payment(self, amount):
        ...
        if not self._should_succeed:
            raise PaymentFailedError("UPI payment failed.")

        transaction_id = self._transaction_id or f"UPI-{uuid4().hex[:8].upper()}"
        return PaymentResult(transaction_id, amount, "UPI")
